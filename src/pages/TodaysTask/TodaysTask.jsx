...
const TodaysTask=()=>{
    ...
    const todayAssignedTasks = tasks?.filter(task=> task.dueDate === formattedDate);
    const assignedToMeTasks = tasks?.filter(task=> task.assignTo === currentUser.displayName && task.priority === "high");
    ...
    if(todayAssignedTasks.length === 0){
        return(
            <div className="flex justify-center flex-col items-center h-screen -mt-20">
                <h2 className="text-4xl font-semibold">No Today Assigned Tasks Found!</h2>
    ...
    return(
        <div className="ps-10 pt-5">
            <div>
                <h2 className="text-3xl font-bold">Today Assigned Tasks</h2>
                <div className="pt-5 overflow-x-auto">
                    <table className="table">
                        <tbody className="flex flex-col">
                            {
                                todayAssignedTasks?.map(task=> <Link
                                    key={task._id}
